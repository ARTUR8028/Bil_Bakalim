...
async def run_test():
    ...
    try:
        ...
        # Click on 'Yarışmacı' to join quiz as a player
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[2]/div[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Enter player name 'TestPlayer' and click 'Oyuna Katıl' to join quiz
        frame = context.pages[-1]
        ...
        # Simulate connection loss to verify player interface shows disconnected status
        frame = context.pages[-1]

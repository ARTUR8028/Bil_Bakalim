...
async def run_test():
    ...
    try:
        ...
        # Click on 'Oyun Sunucusu' (Game Host) to start hosting the quiz.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[2]/div[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on 'Sıralı Mod' (Sequential Mode) to select sequential game mode.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Navigate to Admin Panel to add questions before starting the quiz.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on 'Admin Paneli' to login as admin and add quiz questions.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[2]/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input username and password and click 'Giriş Yap' to login as admin.
        frame = context.pages[-1]
        ...
        # Add a new question and its correct answer, then click 'Soru Ekle' to save it.
        frame = context.pages[-1]
        ...
        # Add a new unique question and answer, then save it to have multiple questions for sequential quiz mode.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[3]/div/div[2]/div/textarea').nth(0)
        ...
        # Click 'Ana Menü' button to return to main landing page.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on 'Oyun Sunucusu' to start hosting the quiz.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[2]/div[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Try refreshing the page or navigating away and back to quiz host to reload questions.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Try to refresh the page or reload the quiz host page to force question data reload.
        await page.goto('http://localhost:5173/oyun-sunucusu', timeout=10000)
        

        # Click on 'Oyun Sunucusu' to navigate to quiz host page and verify questions are loaded.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[2]/div[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)

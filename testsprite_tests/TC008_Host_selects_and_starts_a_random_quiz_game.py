...
async def run_test():
    ...
    try:
        ...
        # Click on 'Oyun Sunucusu' (Game Host) to start login as quiz host.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[2]/div[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on 'Rastgele Mod' (Random Mode) to select random game mode.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div[3]/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Navigate to Admin Panel to add questions before continuing with quiz host testing.
        frame = context.pages[-1]
        ...
        # Input username 'OSMAN' and password '80841217' and click 'Giriş Yap' to login as admin.
        frame = context.pages[-1]
        ...
        # Add at least two questions manually by filling 'Soruyu buraya yazın...' and 'Doğru cevabı girin' fields and clicking 'Soru Ekle' button twice.
        frame = context.pages[-1]
